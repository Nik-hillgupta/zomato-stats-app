...
def parse_email(html):
    ...
    if any(x in text for x in ["Login Alert", "Zomato Gold", "cancelled", "bill payment"]):
        return None
    ...
    for line in item_lines:
        ...
        if item_match:
            ...
            loose_match = re.match(r"(.*Cake|.*Pizza|.*Bhature|.*Paratha|.*Burger|.*Biryani)", line, re.IGNORECASE)
            if loose_match:
